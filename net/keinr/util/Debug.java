...
public class Debug {
     ...
     * @param message the message to log
     */
    public void logRouted(Object message, int distance) {
        if (debug) {
            StackTraceElement call = getTrace(distance); // Get stacktrace for where function was called
    ...
    public void logFunctionRouted(Runnable function, int distance) {
        if (debug) {
            StackTraceElement call = getTrace(distance);
            System.out.println(
    ...
    private static StackTraceElement getTrace(int distance) {
        StackTraceElement[] trace = Thread.currentThread().getStackTrace();
        distance += 3;
        if (distance >= trace.length) {
            distance = trace.length-1;
        }
